...
class TestLanguages < Test::Unit::TestCase
    ...
    def testEiBiLanguageCodeKeys
        ...
        expectedKeys.each do |key|
            ...
            puts "*** Key failed: >#{key}< count: #{count}" unless result
            assert result
            count += 1
        end
    end

    def testEiBiLanguageCodeKeysForEmptyValues
        $eibiLanguageCodeToISO639dash3.keys.each do |key|
            value = $eibiLanguageCodeToISO639dash3[key]
            puts "EiBi key with empty value: #{key}" if (nil == value) || (0 == value.length)
            assert nil != value
